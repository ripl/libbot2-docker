...
RUN \
  ...
  cd /tmp/libbot2 && \
  # download libbot2
  wget --no-check-certificate https://github.com/ripl/libbot2/archive/${LIBBOT2_BRANCH}.tar.gz && \
  tar -zxvf ${LIBBOT2_BRANCH}.tar.gz && \
  cd /tmp/libbot2/libbot2-${LIBBOT2_BRANCH} && \
  ...
  mkdir build && \
  cd build && \
  cmake -DCMAKE_INSTALL_PREFIX=$LIBBOT2_INSTALL_DIR ../ && \
  make -j install && \
  # remove source code
  cd / && \
  rm -rf /tmp/libbot2
